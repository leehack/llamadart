...
| R02 | Complete C05 thinking/budgets, C07 tool auto/required/none and continuation, C10 stop-marker semantics, C11 batching parity, C12 guard/recovery subcases and C02 separate Unicode generation | C09's second dispose/load/generate cycle is implemented in focused lifecycle and release selections. The other six extended obligations remain NOT_RUN until positive fixtures and typed negative/version-skew checks are qualified. |
| R03 | Core focused selection and versioned metadata implemented; extend the catalog as future model/media packs land | Schema 2 binds case/feature versions, resolved prompts/tools/predicates and fixture hashes; omitted cases have explicit reasons. `tiny-gguf-lifecycle` is the runnable focused example. Future pack media hashes and reference qualifications remain with R04/R05. |
