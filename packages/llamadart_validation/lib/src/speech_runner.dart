...
abstract interface class SpeechSynthesisInterruptAdapter {
  ...
  /// Runs [speechDecodeCancelReferenceRuns] syntheses capped at
  /// [speechDecodeCancelFrameCap] frames, then another cancelled
  /// [speechDecodeCancelLeadFraction] of their shortest audio decode time after
  /// the progress event that reports the cap.
  ///
  /// Reports `frame_cap`; `uncapped_frames`, generated by the latest normal
  /// synthesis; `references`, each with its `frames`, `truncated` and
  /// `decode_ms`, from that progress event to its terminal state; and, for the
  /// cancelled synthesis, `frames_before_cancel`,
  /// `cancel_after_decode_start_ms`, `cancel_in_flight`, `completion_state`,
  /// `final_events` and `cancel_latency_ms`.
...
class PublicSpeechValidationAdapter
  ...
  Future<Map<String, Object?>> executeDecodeCancel() async {
    final uncapped =
        _lastFramesGenerated ??
        (throw StateError('No measured synthesis to cap'));
    final references = <Map<String, Object?>>[];
    for (var run = 0; run < speechDecodeCancelReferenceRuns; run++) {
      final reference = await _cappedSynthesis();
      final capMs = reference.capEventMs;
      references.add({
        'frames': reference.completion.result?.framesGenerated,
        'truncated': reference.completion.result?.truncated,
        'decode_ms': capMs == null ? null : reference.doneMs - capMs,
      });
    }
    final measured = <String, Object?>{
    ...
    final decodes = references.map((reference) => reference['decode_ms']);
    if (decodes.any((decode) => decode == null)) return measured;
    final decodeMs = decodes.cast<double>().reduce(math.min);
...
/// Uncancelled capped syntheses `decode_cancel` times; the shortest audio
/// decode among them is the reference.
const speechDecodeCancelReferenceRuns = 2;

/// How far into the audio decode `decode_cancel` cancels, as a fraction of the
/// reference decode time.
const speechDecodeCancelLeadFraction = 0.25;

/// Ceiling on the `decode_cancel` cancellation latency, as a fraction of the
/// reference decode time left when it cancels. Latency equal to it passes.
///
/// The references and the cancelled synthesis are separate runs, so without
/// chunk-boundary cancellation a cancelled synthesis whose decode runs fast
/// enough can still pass.
const speechDecodeCancelRemainderBudget = 0.5;
...
Map<String, Object?> _decodeCancelOutcome(Map<String, Object?> result) {
  ...
  if (cap != speechDecodeCancelFrameCap ||
    ...
    throw StateError('The frame cap does not truncate the synthesis');
  }
  ...
  if (references is! List ||
    ...
    throw StateError('A reference synthesis did not stop at the frame cap');
  }
  final decode = references
      .map((reference) => (reference as Map)['decode_ms'] as num)
      .reduce(math.min);
  final lead = result['cancel_after_decode_start_ms'];
  if (lead is! num ||
      !lead.isFinite ||
      lead < decode * speechDecodeCancelLeadFraction ||
      lead >= decode) {
    throw StateError('Cancellation did not land inside the reference decode');
  }
  if (result['frames_before_cancel'] != cap ||
      result['cancel_in_flight'] != true) {
    throw StateError('Cancellation did not reach a decoding synthesis');
  }
  if (result['completion_state'] != 'cancelled' ||
      result['final_events'] != 0) {
    throw StateError('Decode cancellation emitted a final result');
  }
  final latency = result['cancel_latency_ms'];
  if (latency is! num || !latency.isFinite || latency < 0) {
    throw StateError('Cancellation latency was not measured');
  }
  final remainder = decode - lead;
  final budget = remainder * speechDecodeCancelRemainderBudget;
  return {
    ...result,
    'lead_fraction': speechDecodeCancelLeadFraction,
    'reference_decode_ms': decode,
    'reference_remainder_ms': remainder,
    'remainder_budget': speechDecodeCancelRemainderBudget,
    'budget_ms': budget,
    'predicate_passed': latency <= budget,
  };
}
...
/// synthesis after the reload passes. `decode_cancel` passes only if its
/// latency is at most [speechDecodeCancelRemainderBudget] of the decode time
/// the reference had left.
///
