...
- `speculativeDecoding` / `speculativeDecodingConfig`: opt-in backend-native
  ...
  speculation without a draft model. llamadart currently supports
  ngram-simple `draftTokenMax <= 2`; deeper drafts can diverge from
  non-speculative decoding on some model/backend combinations. WebGPU and
  LiteRT-LM web reject speculative decoding until their speculative paths are
  implemented.
