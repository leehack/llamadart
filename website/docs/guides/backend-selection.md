...
its level was `debug`, `info`, or `warn` before the backend worker started;
the default `none` suppresses them, and the native log level does not gate
them. When no directory is passed to the
